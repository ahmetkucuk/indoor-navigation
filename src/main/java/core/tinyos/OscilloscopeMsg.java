...
import model.LightIntensityMeasurement;
...
public class OscilloscopeMsg extends net.tinyos.message.Message {
     ...
     * message type name and the non-indexed field values.
     */
    public LightIntensityMeasurement toLightIntensityResponse() {
        LightIntensityMeasurement response = new LightIntensityMeasurement();
        try {
            response.setVersion(get_version());
        } catch (ArrayIndexOutOfBoundsException aioobe) { /* Skip field */ }
        try {
            response.setInterval(get_interval());
        } catch (ArrayIndexOutOfBoundsException aioobe) { /* Skip field */ }
        try {
        ...
        try {
            int[] readings = new int[NUMBER_OF_READINGS];
            for (int i = 0; i < NUMBER_OF_READINGS; i++) {
                readings[i] = (getElement_readings(i) & 0xffff) - get_secret();
            }
    ...
    /**
     * Return the secret (as a int) of the field 'secret'
     */
    public int get_secret() {
        return (int)getUIntBEElement(offsetBits_secret(), 16);
    }
